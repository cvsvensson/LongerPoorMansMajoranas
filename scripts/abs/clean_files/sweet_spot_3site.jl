...
exps = range(0.1, 4, 5)
_homogeneous_ss2 = find_sweet_spot((f2, f2!, cache2), c2, Hδϕ_Hε(); exps, MaxTime=2, target, minexcgap=0, alg=BestOf(best_algs()[1:end-1]))
...
for δε2 in δε2s
    ...
    #WARNING: This is a hack to get one of the branches of the sweet spot
...
for k in eachindex(Ezs)
    ...
    alg = BestOf(best_algs()[1:end-1])
    f, f!, cache = hamfunc(Rδϕ_Rε(), c, fixedparams)
    minexcgap = sweet_spots_Ez3[k].optsol.excgap * 0.8
    δϕ0, ε0 = sweet_spots_Ez3[k].sol |> collect
    prob = OptProb(; hamfunc=x -> f!(cache, x), basis=c, optparams=Rδϕ_Rε(), target)
    ranges = [(0.0, 1.0pi), ε0 .+ 1 .* (-1, 1), ε0 .+ 1 .* (-1, 1)]
    initials = [δϕ0, ε0, ε0]
...
##
c_main = 1
p_Ez_excgap = plot(Ezs, [x.optsol.excgap for x in sweet_spots_Ez3], markers=true, ylabel="excgap", xlabel="Ez", ylims=(0, 0.8), c=c_main)
plot!(Ezs, [LDbdg(x.optsol) for x in sweet_spots_Ez2], markers=true)
plot!(Ezs, [LDbdg(x.optsol) for x in inhomo_sols], markers=true, ls=:dash, c=c_main)
p_Ez_LD = plot(Ezs, [LDbdg(x.optsol) for x in sweet_spots_Ez3], markers=true, ylabel="LDbdg", xlabel="Ez", clims=(0, 1.5), c=c_main)
plot!(Ezs, [LDbdg(x.optsol) for x in sweet_spots_Ez2], markers=true)
plot!(Ezs, [LDbdg(x.optsol) for x in inhomo_sols], markers=true, ls=:dash, c=c_main)
p_Ez_gap = plot(Ezs, [norm(x.gradient) for x in sweet_spots_Ez3], markers=true, ylabel="|gradient|", xlabel="Ez", clims=(0, 1.5), c=c_main)
...
plot(p_Ez_excgap, p_Ez_LD, p_Ez_gap, p_Ez_hess)

##
plot(Ezs, stack(map((h, i) -> [h.sol[1] - i.sol[1], h.sol[2] - i.sol[2], h.sol[2] - i.sol[3]], sweet_spots_Ez3, inhomo_sols))', marker=true, label=["δϕ" "ε1" "ε2"], ylabel="difference", xlabel="Ez")
##
