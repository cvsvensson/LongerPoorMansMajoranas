...
fig = with_theme(theme_latexfonts()) do
    ...
    axislegend(ax, f_ss, [L"\mathrm{mLD}_0 \text{ (Phase branch)}", L"\mathrm{mLD}", L"\mathrm{mLD}_0 \text{ (Level branch)}", "Sweet spot"][1:length(f_ss)], position=:rb, labelsize=13, titlesize=13)
...
fig_pert = with_theme(theme_latexfonts()) do
    ...
    εs = pert_data_dict["εs"]
    δϕs = pert_data_dict["δϕs"]
    ...
    ax2 = Axis(g[1, 2]; title=L"H_1^\text{eff} + H_2^\text{eff}", xlabel=L"ε/Δ", ylabel=L"δϕ", yticks=(pi * [0, 1 / 2, 1], [L"0", L"\frac{\pi}{2}", L"π"]), xticks)
    linkaxes!(ax, ax1, ax2)
    hideydecorations!(ax)
