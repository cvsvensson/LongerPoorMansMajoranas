zerothorder_JUexp = :(((adjoint(a[n]) * a[n]) * (μ[n] ^ 2 + -1 * (Vz * sqrt(Δ ^ 2 + μ[n] ^ 2)) + Δ * sqrt(-1 * μ[n] + sqrt(Δ ^ 2 + μ[n] ^ 2)) * sqrt(μ[n] + sqrt(Δ ^ 2 + μ[n] ^ 2)))) * sqrt(Δ ^ 
2 + μ[n] ^ 2) ^ -1);

firstorder_hopping_JUexp = :((tso * (a[n] * a[1 + n]) * (ℯ ^ ((1im) * ϕ[1 + n]) * sqrt(μ[n] + sqrt(Δ ^ 2 + μ[n] ^ 2)) * sqrt(-1 * μ[1 + n] + sqrt(Δ ^ 2 + μ[1 + n] ^ 2)) + ℯ ^ ((1im) * ϕ[n]) * sqrt(-1 
* μ[n] + sqrt(Δ ^ 2 + μ[n] ^ 2)) * sqrt(μ[1 + n] + sqrt(Δ ^ 2 + μ[1 + n] ^ 2))) + -1 * (tso * (adjoint(a[n]) * adjoint(a[1 + n])) * (ℯ ^ ((1im) * ϕ[n]) * sqrt(μ[n] + sqrt(Δ 
^ 2 + μ[n] ^ 2)) * sqrt(-1 * μ[1 + n] + sqrt(Δ ^ 2 + μ[1 + n] ^ 2)) + ℯ ^ ((1im) * ϕ[1 + n]) * sqrt(-1 * μ[n] + sqrt(Δ ^ 2 + μ[n] ^ 2)) * sqrt(μ[1 + n] + sqrt(Δ ^ 2 + μ[1 + 
n] ^ 2)))) + t * (adjoint(a[n]) * a[1 + n]) * (ℯ ^ ((1im) * ϕ[1 + n]) * sqrt(-1 * μ[n] + sqrt(Δ ^ 2 + μ[n] ^ 2)) * sqrt(-1 * μ[1 + n] + sqrt(Δ ^ 2 + μ[1 + n] ^ 2)) + -1 * (ℯ ^ ((1im) * ϕ[n]) * sqrt(μ[n] + sqrt(Δ ^ 2 + μ[n] ^ 2)) * sqrt(μ[1 + n] + sqrt(Δ ^ 2 + μ[1 + n] ^ 2)))) + t * (adjoint(a[1 + n]) * a[n]) * (ℯ ^ ((1im) * ϕ[n]) * sqrt(-1 * μ[n] + sqrt(Δ ^ 2 + μ[n] ^ 2)) * sqrt(-1 * μ[1 + n] + sqrt(Δ ^ 2 + μ[1 + n] ^ 2)) + -1 * (ℯ ^ ((1im) * ϕ[1 + n]) * sqrt(μ[n] + sqrt(Δ ^ 2 + μ[n] ^ 2)) * sqrt(μ[1 + n] + sqrt(Δ ^ 2 + μ[1 + n] ^ 2))))) * (2 * ℯ ^ (((1im) * 2 ^ -1) * (ϕ[n] + ϕ[1 + n])) * (Δ ^ 2 + μ[n] ^ 2) ^ (1 * 4 ^ -1) * (Δ ^ 2 + μ[1 + n] ^ 2) ^ (1 * 4 ^ -1)) ^ -1);
...
(2 * ϕ[2] + ϕ[3])) * (t ^ 2 + tso ^ 2) * sqrt(-1 * μ[1] + sqrt(Δ ^ 2 + μ[1] ^ 2)) * sqrt(μ[1] + sqrt(Δ ^ 2 + μ[1] ^ 2)) * sqrt(-1 * μ[2] + sqrt(Δ ^ 2 + μ[2] ^ 2)) * sqrt(μ[2] + sqrt(Δ ^ 2 + μ[2] ^ 2)) * sqrt(Δ ^ 2 + μ[3] ^ 2) + ℯ ^ ((1im) * (ϕ[1] + 2 * ϕ[2])) * (t ^ 2 + tso ^ 2) * sqrt(Δ ^ 2 + μ[1] ^ 2) * sqrt(-1 * μ[2] + sqrt(Δ ^ 2 + μ[2] ^ 2)) * sqrt(μ[2] + sqrt(Δ ^ 2 + μ[2] ^ 2)) * sqrt(-1 * μ[3] + sqrt(Δ ^ 2 + μ[3] ^ 2)) * sqrt(μ[3] + sqrt(Δ ^ 2 + μ[3] ^ 2)) + ℯ ^ ((1im) * (ϕ[1] + 2 * ϕ[3])) * (t ^ 2 + tso ^ 2) * sqrt(Δ ^ 2 + μ[1] ^ 2) * sqrt(-1 * μ[2] + sqrt(Δ ^ 2 + μ[2] ^ 2)) * sqrt(μ[2] + sqrt(Δ ^ 2 + μ[2] ^ 2)) * sqrt(-1 * μ[3] + sqrt(Δ ^ 2 + μ[3] ^ 2)) * sqrt(μ[3] + sqrt(Δ ^ 2 + μ[3] ^ 2)) + ℯ ^ ((1im) * (ϕ[1] + ϕ[2] + ϕ[3])) * (t ^ 2 * (4 * sqrt(Δ ^ 2 + μ[1] ^ 2) * sqrt(Δ ^ 2 + μ[2] ^ 2) * sqrt(Δ ^ 2 + μ[3] ^ 2) + -1 * (2 * μ[2] * (sqrt(Δ ^ 2 
+ μ[1] ^ 2) * μ[3] + μ[1] * sqrt(Δ ^ 2 + μ[3] ^ 2)))) + -1 * (2 * tso ^ 2 * (2 * sqrt(Δ ^ 2 + μ[1] ^ 2) * sqrt(Δ ^ 2 + μ[2] ^ 2) * sqrt(Δ ^ 2 + μ[3] ^ 2) + μ[2] * (sqrt(Δ ^ 
2 + μ[1] ^ 2) * μ[3] + μ[1] * sqrt(Δ ^ 2 + μ[3] ^ 2))))))) * (4 * ℯ ^ ((1im) * (ϕ[1] + ϕ[2] + ϕ[3])) * Vz * sqrt(Δ ^ 2 + μ[1] ^ 2) * sqrt(Δ ^ 2 + μ[2] ^ 2) * sqrt(Δ ^ 2 + μ[3] ^ 2)) ^ -1)
...
##
perturbative_hamiltonian(a; Δ, μ, δϕ, tso, t, Ez) = zerothorder_perturbation(a; Δ=Δ, μ=μ, Ez=Ez) + firstorder_perturbation(a; Δ=Δ, μ=μ, δϕ=δϕ, tso=tso, t=t) + secondorder_perturbation(a; Δ=Δ, μ=μ, δϕ=δϕ, tso=tso, t=t, Ez=Ez)
perturbative_hamiltonian(a; Δ=1, μ=1:3, δϕ=1:2, tso=1/100000, t=1 / 10000, Ez=3) - zerothorder_perturbation(a; Δ=1, μ=1:3, Ez=3) |> norm
